...
void main() 
{
	...
	if(dataready )			// data is received and ready to be procssed 
	{
		...
		{
		case 0x50: RELAY1 = 1;		//Toggle relay 1	
				   break;
		case 0xD8: RELAY2 = 1;		//Toggle relay 2
				   break;
		case 0xF8: RELAY3 = 1;		//Toggle relay 3
				   break;
		case 0x30: RELAY4 = 1;		//Toggle relay 4
				   break;
...
tdiff = ((timer<<8)+TMR0) ;     // calculate how much time has been passed since last interrupt ,because we inrement timer variable on every over flow on TMR0 physcial timer to we have to get the timer value up to its significance before addition 
...
necpoj == 70 	  this pojsition is used for repeat code 70 means we just passed 9ms + 2.25 ms pluse and only 
*/
...
{												// At the very first edge (necpoj==0)  this conditon will always false and the false block of this if will bring the state machine (necpoj) to position 1(position 1 means 9ms leading pulse has started now we have to wait for 4.5ms start pulse to occur) 
	...
	if(necpoj==1 || necpoj==2)					// when we are hear it means 9ms leding pulse has ended and now we are necpoj=1 or necpoj=2
	{
			...
			PREPULSE   	= TICKS2ms;				// PREPULSE for 3rd pulse 2ms		, so that we can detect repeat code
		
			}
			else if((pin==0)&& (necpoj ==2))
			{
			if((tdiff>TICKS1o8ms)&&(tdiff<TICKS3ms) )		//check if it is a repeat code 
			{
			necpoj=70;				
			TIMEOUT 	= TICKS0o85ms;  			// yes it is 
			PREPULSE   	= TICKS0o2ms; 	
			}else if((tdiff>TICKS3ms)&&(tdiff<TICKS5o5ms) )		
			{
			 necpoj++;		
	...
	else if(necpoj>2 && necpoj<70)							//now we are picking the data 	
	{	
